...
describe('template manifests and rendering', () => {
  ...
  it('rejects duplicate JSON keys and unsafe YAML manifest features', () => {
    const duplicate = new TextEncoder().encode('{"version":"1.0.0-rc.3","version":"1.0.0-rc.3"}');
    const anchored = new TextEncoder().encode(
      'version: &version "1.0.0-rc.3"\nkind: "domain.md/template-manifest"\n',
    );
  ...
  it('renders deterministically and strips template markers', async () => {
    ...
    const constitution = parsed.document?.frontmatter.constitution as
      { subject?: string; subject_profile?: { identity?: string[] } } | undefined;
    expect(constitution?.subject).toBe(domain?.id);
    expect(constitution?.subject_profile?.identity).toContain(domain?.id);
