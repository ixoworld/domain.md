...
try {
  ...
  const smokeProgram = [
    ...
    "if (!api.getSpecification().includes('1.0.0-rc.3')) throw new Error('spec asset mismatch');",
    "if (api.getSchema().$id !== 'urn:ixo:domain-md:schema:1.0.0-rc.3') throw new Error('schema asset mismatch');",
